...
)
{
  ...
  try
  {
    ...
    std::string wss = _my_url + "/?token=" + _my_token + "&roomId=" + _my_room;
    //Get connection
...
bool EvercastWebsocketClientImpl::trickle(const std::string &mid, int index, const std::string &candidate, bool last)
{
  try
  {
    //Check if it is last
