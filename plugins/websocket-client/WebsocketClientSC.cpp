...
{
    return new WebsocketClientSC();
}

WebsocketClientSC::WebsocketClientSC()
{
...
bool WebsocketClientSC::connect(std::string url, long long room, std::string username, std::string token, WebsocketClient::Listener* listener)
{
            ...
            if (id.compare("success") == 0)
            {
                ...
                if (!logged)
                {
                    ...
                    thread_keepAlive = std::thread([&]() {
                        WebsocketClientSC::keepConnectionAlive();
                    });
                }else {
                    handle_id = data["id"];
...
bool WebsocketClientSC::open(const std::string &sdp)
{
    try
    {
        //Login command
...
bool WebsocketClientSC::trickle(const std::string &mid, int index, const std::string &candidate, bool last)
{
    try
    {
        //Check if it is last
...
void WebsocketClientSC::keepConnectionAlive()
{
...
bool WebsocketClientSC::disconnect(bool wait)
{
