...
prepare_macos_bundle() {
    ...
    if [ -d ./OBS-WebRTC.app ]; then /bin/rm -rf ./OBS-WebRTC.app; fi

    hr "Preparing OBS-WebRTC.app bundle"
    step "Copy binary and plugins..."
    /bin/mkdir -p OBS-WebRTC.app/Contents/MacOS
    /bin/mkdir OBS-WebRTC.app/Contents/PlugIns
    ...
    /bin/cp "${CI_SCRIPTS}/app/Info.plist" ./OBS-WebRTC.app/Contents
    ...
    /bin/cp "${CI_SCRIPTS}/app/OBSPublicDSAKey.pem" ./OBS-WebRTC.app/Contents/Resources

    step "Set bundle meta information..."
    /usr/bin/plutil -insert CFBundleVersion -string "${OBS_VERSION}" ./OBS-WebRTC.app/Contents/Info.plist
    /usr/bin/plutil -insert CFBundleShortVersionString -string "${MACOSX_BUNDLE_SHORT_VERSION_STRING}" ./OBS-WebRTC.app/Contents/Info.plist
    /usr/bin/plutil -insert OBSFeedsURL -string https://obsproject.com/osx_update/feeds.xml ./OBS-WebRTC.app/Contents/Info.plist
    /usr/bin/plutil -insert SUFeedURL -string https://obsproject.com/osx_update/stable/updates.xml ./OBS-WebRTC.app/Contents/Info.plist
    /usr/bin/plutil -insert SUPublicDSAKeyFile -string OBSPublicDSAKey.pem ./OBS-WebRTC.app/Contents/Info.plist
}
...
codesign_bundle() {
    ...
    /usr/bin/xattr -crs ./OBS-WebRTC.app
    ...
    /bin/echo -n "${COLOR_ORANGE}"
    /bin/echo "ls -lR ./OBS-WebRTC.app/Contents/"
    ls -lR ./OBS-WebRTC.app/Contents
