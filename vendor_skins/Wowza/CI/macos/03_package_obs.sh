...
package_obs() {
    ...
    if [ "${DMG_NAME}" ]; then
        ...
        /usr/bin/codesign --force --sign "${WOWZA_CODESIGN_IDENT:--}" "${BUILD_DIR}/${FILE_NAME}"
    else
...
notarize_obs() {
    ...
    if [ "${NOTARIZE_IMAGE}" ]; then
        ...
        if [ -f "${OBS_IMAGE}" ]; then
            ...
            error "No notarization application bundle ('Wowza-OBS-Real-Time.app') or disk image ('${NOTARIZE_IMAGE:-${FILE_NAME}}') found"
            return
        fi
    fi
    ...
    if [ "$?" -eq 0 ]; then
        ...
        # CODESIGN_IDENT_SHORT=$(echo "${WOWZA_CODESIGN_IDENT}" | /usr/bin/sed -En "s/.+\((.+)\)/\1/p")
        echo -n "${COLOR_ORANGE}"
...
package-obs-standalone() {
    ...
    if [ -z "${NOTARIZE_IMAGE}" -a -z "${NOTARIZE_BUNDLE}" ]; then
        if [ "${ARCH}" = "arm64" ]; then
            FILE_NAME="Wowza-OBS-Real-Time-${OBS_VERSION}-macos-arm64.dmg"
        elif [ "${ARCH}" = "universal" ]; then
            FILE_NAME="Wowza-OBS-Real-Time-${OBS_VERSION}-macos.dmg"
        else
            FILE_NAME="Wowza-OBS-Real-Time-${OBS_VERSION}-macos-x86_64.dmg"
        fi
