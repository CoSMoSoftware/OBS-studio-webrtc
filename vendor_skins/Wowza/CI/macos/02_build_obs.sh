...
_configure_obs() {
    ...
    cmake -S . -B ${BUILD_DIR} -G ${GENERATOR} \
        ...
        -DOBS_BUNDLE_CODESIGN_IDENTITY="${WOWZA_CODESIGN_IDENT:--}" \
        ${YOUTUBE_OPTIONS} \
        ${TWITCH_OPTIONS} \
        ${RESTREAM_OPTIONS} \
        ${UNITTEST_OPTIONS} \
...
_backup_artifacts() {
    ...
    if [ -d "${BUILD_DIR}" ]; then
        ...
        if [ -d "${BUILD_DIR}/install/Wowza-OBS-Real-Time.app" ]; then
            step "Back up Wowza-OBS-Real-Time.app..."
            ensure_dir "${NIGHTLY_DIR}"
            /bin/mv "${CHECKOUT_DIR}/${BUILD_DIR}/install/Wowza-OBS-Real-Time.app" "${NIGHTLY_DIR}/"
...
print_usage() {
    echo -e "Usage: ${0}\n" \
            ...
            "-b, --bundle                   : Create relocatable OBS application bundle in build directory (default: build/install/Wowza-OBS-Real-Time.app)\n" \
