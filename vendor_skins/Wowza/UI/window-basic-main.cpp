...
static void AddExtraModulePaths()
{
	...
	int ret = GetConfigPath(
		base_module_dir, sizeof(base_module_dir),
		(std::string(CONFIG_DIR) + "/plugins/%module%.plugin").c_str());
#else
	int ret = GetConfigPath(
		base_module_dir, sizeof(base_module_dir),
		(std::string(CONFIG_DIR) + "/plugins/%module%").c_str());
...
void OBSBasic::UploadLog(const char *subdir, const char *file, const bool crash)
{
	...
	stringstream ss;
	ss << "Wowza-OBS-Real-Time " << App()->GetVersionString() << " log file uploaded at "
...
{
	QUrl url = QUrl("https://www.wowza.com", QUrl::TolerantMode);
	QDesktopServices::openUrl(url);
}
...
{
	char path[512];
	int ret = GetConfigPath(path, 512, std::string(CONFIG_DIR).c_str());
	if (ret <= 0)
		return;
