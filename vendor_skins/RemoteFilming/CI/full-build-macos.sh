...
## SET UP ENVIRONMENT ##
PRODUCT_NAME="RemoteFilmingStudio"
...
configure_obs_build() {
    ...
    if [ -d ./RemoteFilmingStudio.app ]; then
        ensure_dir "${NIGHTLY_DIR}"
        mv ../${BUILD_DIR}_${VENDOR}/RemoteFilmingStudio.app .
        info "You can find RemoteFilmingStudio.app in ${NIGHTLY_DIR}"
    fi
    ensure_dir "${CHECKOUT_DIR}/${BUILD_DIR}_${VENDOR}"
...
bundle_dylibs() {
    ...
    if [ ! -d ./RemoteFilmingStudio.app ]; then
        error "No RemoteFilmingStudio.app bundle found"
        exit 1
    fi
    ...
    ${CI_SCRIPTS}/app/dylibbundler -cd -of -a ./RemoteFilmingStudio.app -q -f \
        -s ./RemoteFilmingStudio.app/Contents/MacOS \
        -s ./rundir/${CI_BUILD_TYPE}/bin/ \
        -x ./RemoteFilmingStudio.app/Contents/PlugIns/coreaudio-encoder.so \
        -x ./RemoteFilmingStudio.app/Contents/PlugIns/decklink-ouput-ui.so \
        -x ./RemoteFilmingStudio.app/Contents/PlugIns/decklink-captions.so \
        -x ./RemoteFilmingStudio.app/Contents/PlugIns/frontend-tools.so \
        -x ./RemoteFilmingStudio.app/Contents/PlugIns/image-source.so \
        -x ./RemoteFilmingStudio.app/Contents/PlugIns/mac-avcapture.so \
        -x ./RemoteFilmingStudio.app/Contents/PlugIns/mac-capture.so \
        -x ./RemoteFilmingStudio.app/Contents/PlugIns/mac-decklink.so \
        -x ./RemoteFilmingStudio.app/Contents/PlugIns/mac-syphon.so \
        -x ./RemoteFilmingStudio.app/Contents/PlugIns/mac-vth264.so \
        -x ./RemoteFilmingStudio.app/Contents/PlugIns/mac-virtualcam.so \
        -x ./RemoteFilmingStudio.app/Contents/PlugIns/obs-ffmpeg.so \
        -x ./RemoteFilmingStudio.app/Contents/PlugIns/obs-filters.so \
        -x ./RemoteFilmingStudio.app/Contents/PlugIns/obs-transitions.so \
        -x ./RemoteFilmingStudio.app/Contents/PlugIns/obs-vst.so \
        -x ./RemoteFilmingStudio.app/Contents/PlugIns/rtmp-services.so \
        -x ./RemoteFilmingStudio.app/Contents/MacOS/obs-ffmpeg-mux \
        -x ./RemoteFilmingStudio.app/Contents/MacOS/obslua.so \
        -x ./RemoteFilmingStudio.app/Contents/PlugIns/obs-x264.so \
...
#        -x ./RemoteFilmingStudio.app/Contents/PlugIns/obs-browser.so
    ...
    cp -R /tmp/obsdeps/lib/QtNetwork.framework ./RemoteFilmingStudio.app/Contents/Frameworks
    chmod -R +w ./RemoteFilmingStudio.app/Contents/Frameworks/QtNetwork.framework
    rm -r ./RemoteFilmingStudio.app/Contents/Frameworks/QtNetwork.framework/Headers
    rm -r ./RemoteFilmingStudio.app/Contents/Frameworks/QtNetwork.framework/Versions/5/Headers/
    chmod 644 ./RemoteFilmingStudio.app/Contents/Frameworks/QtNetwork.framework/Versions/5/Resources/Info.plist
    install_name_tool -id @executable_path/../Frameworks/QtNetwork.framework/Versions/5/QtNetwork ./RemoteFilmingStudio.app/Contents/Frameworks/QtNetwork.framework/Versions/5/QtNetwork
    install_name_tool -change /tmp/obsdeps/lib/QtCore.framework/Versions/5/QtCore @executable_path/../Frameworks/QtCore.framework/Versions/5/QtCore ./RemoteFilmingStudio.app/Contents/Frameworks/QtNetwork.framework/Versions/5/QtNetwork
}
...
install_frameworks() {
    ...
    step "Copy Framework..."
    cp -R "${DEPS_BUILD_DIR}/cef_binary_${MACOS_CEF_BUILD_VERSION}_macosx64/Release/Chromium Embedded Framework.framework" ./RemoteFilmingStudio.app/Contents/Frameworks/
    chown -R $(whoami) ./RemoteFilmingStudio.app/Contents/Frameworks/
}
...
prepare_macos_bundle() {
    ...
    if [ -d ./RemoteFilmingStudio.app ]; then rm -rf ./RemoteFilmingStudio.app; fi

    hr "Preparing RemoteFilmingStudio.app bundle"
    step "Copy binary and plugins..."
    mkdir -p RemoteFilmingStudio.app/Contents/MacOS
    mkdir RemoteFilmingStudio.app/Contents/PlugIns
    mkdir RemoteFilmingStudio.app/Contents/Resources
    mkdir RemoteFilmingStudio.app/Contents/Frameworks

    cp rundir/${CI_BUILD_TYPE}/bin/rfs ./RemoteFilmingStudio.app/Contents/MacOS
    cp rundir/${CI_BUILD_TYPE}/bin/obs-ffmpeg-mux ./RemoteFilmingStudio.app/Contents/MacOS
    cp rundir/${CI_BUILD_TYPE}/bin/libobsglad.0.dylib ./RemoteFilmingStudio.app/Contents/MacOS
#    cp -R "rundir/${CI_BUILD_TYPE}/bin/OBS Helper.app" "./RemoteFilmingStudio.app/Contents/Frameworks/OBS Helper.app"
#    cp -R "rundir/${CI_BUILD_TYPE}/bin/OBS Helper (GPU).app" "./RemoteFilmingStudio.app/Contents/Frameworks/OBS Helper (GPU).app"
#    cp -R "rundir/${CI_BUILD_TYPE}/bin/OBS Helper (Plugin).app" "./RemoteFilmingStudio.app/Contents/Frameworks/OBS Helper (Plugin).app"
#    cp -R "rundir/${CI_BUILD_TYPE}/bin/OBS Helper (Renderer).app" "./RemoteFilmingStudio.app/Contents/Frameworks/OBS Helper (Renderer).app"
    cp -R rundir/${CI_BUILD_TYPE}/data ./RemoteFilmingStudio.app/Contents/Resources
    cp ${CI_SCRIPTS}/app/AppIcon.icns ./RemoteFilmingStudio.app/Contents/Resources
    cp -R rundir/${CI_BUILD_TYPE}/obs-plugins/ ./RemoteFilmingStudio.app/Contents/PlugIns
    cp ${CI_SCRIPTS}/app/Info.plist ./RemoteFilmingStudio.app/Contents
    # Scripting plugins are required to be placed in same directory as binary
    if [ -d ./RemoteFilmingStudio.app/Contents/Resources/data/obs-scripting ]; then
        mv ./RemoteFilmingStudio.app/Contents/Resources/data/obs-scripting/obslua.so ./RemoteFilmingStudio.app/Contents/MacOS/
        # mv ./RemoteFilmingStudio.app/Contents/Resources/data/obs-scripting/_obspython.so ./RemoteFilmingStudio.app/Contents/MacOS/
        # mv ./RemoteFilmingStudio.app/Contents/Resources/data/obs-scripting/obspython.py ./RemoteFilmingStudio.app/Contents/MacOS/
        rm -rf ./RemoteFilmingStudio.app/Contents/Resources/data/obs-scripting/
    fi

    bundle_dylibs
#    install_frameworks

    cp ${CI_SCRIPTS}/app/OBSPublicDSAKey.pem ./RemoteFilmingStudio.app/Contents/Resources

    step "Set bundle meta information..."
    plutil -insert CFBundleVersion -string "${OBS_VERSION}" ./RemoteFilmingStudio.app/Contents/Info.plist
    plutil -insert CFBundleShortVersionString -string "${MACOSX_BUNDLE_SHORT_VERSION_STRING}" ./RemoteFilmingStudio.app/Contents/Info.plist
    # plutil -insert OBSFeedsURL -string https://obsproject.com/osx_update/feeds.xml ./RemoteFilmingStudio.app/Contents/Info.plist
    # plutil -insert SUFeedURL -string https://obsproject.com/osx_update/stable/updates.xml ./RemoteFilmingStudio.app/Contents/Info.plist
    # plutil -insert SUPublicDSAKeyFile -string OBSPublicDSAKey.pem ./RemoteFilmingStudio.app/Contents/Info.plist
}
...
prepare_macos_image() {
    ...
    echo -n "${COLOR_ORANGE}"
    dmgbuild "RemoteFilmingStudio ${OBS_VERSION}" "${FILE_NAME}" -s ./settings.json
    echo -n "${COLOR_RESET}"
...
codesign_bundle() {
    ...
    xattr -crs ./RemoteFilmingStudio.app

    read_codesign_ident

    # step "Code-sign Sparkle framework..."
    # echo -n "${COLOR_ORANGE}"
    # codesign --force --options runtime --sign "${CODESIGN_IDENT}" "./RemoteFilmingStudio.app/Contents/Frameworks/Sparkle.framework/Versions/A/Resources/Autoupdate.app/Contents/MacOS/fileop"
    # codesign --force --options runtime --sign "${CODESIGN_IDENT}" "./RemoteFilmingStudio.app/Contents/Frameworks/Sparkle.framework/Versions/A/Resources/Autoupdate.app/Contents/MacOS/Autoupdate"
...
#    echo -n "${COLOR_ORANGE}"
#    codesign --force --timestamp --options runtime --sign "${CODESIGN_IDENT}" "./RemoteFilmingStudio.app/Contents/Frameworks/Chromium Embedded Framework.framework/Libraries/libEGL.dylib"
#    codesign --force --timestamp --options runtime --sign "${CODESIGN_IDENT}" "./RemoteFilmingStudio.app/Contents/Frameworks/Chromium Embedded Framework.framework/Libraries/libGLESv2.dylib"
#    codesign --force --timestamp --options runtime --sign "${CODESIGN_IDENT}" "./RemoteFilmingStudio.app/Contents/Frameworks/Chromium Embedded Framework.framework/Libraries/libswiftshader_libEGL.dylib"
#    codesign --force --timestamp --options runtime --sign "${CODESIGN_IDENT}" "./RemoteFilmingStudio.app/Contents/Frameworks/Chromium Embedded Framework.framework/Libraries/libswiftshader_libGLESv2.dylib"
#    codesign --force --timestamp --options runtime --sign "${CODESIGN_IDENT}" "./RemoteFilmingStudio.app/Contents/Frameworks/Chromium Embedded Framework.framework/Libraries/libvk_swiftshader.dylib"
#    codesign --force --timestamp --options runtime --sign "${CODESIGN_IDENT}" --deep "./RemoteFilmingStudio.app/Contents/Frameworks/Chromium Embedded Framework.framework"
    echo -n "${COLOR_RESET}"

    step "Code-sign OBS code..."
    echo -n "${COLOR_ORANGE}"
    codesign --force --timestamp --options runtime --sign "${CODESIGN_IDENT}" "./RemoteFilmingStudio.app/Contents/Resources/data/obs-mac-virtualcam.plugin/Contents/MacOS/obs-mac-virtualcam"
    codesign --force --timestamp --options runtime --entitlements "${CI_SCRIPTS}/app/entitlements.plist" --sign "${CODESIGN_IDENT}" --deep ./RemoteFilmingStudio.app
#    codesign --force --timestamp --options runtime --sign "${CODESIGN_IDENT}" --deep "./RemoteFilmingStudio.app/Contents/Frameworks/OBS Helper.app"
...
#    codesign --force --timestamp --options runtime --entitlements "${CI_SCRIPTS}/helpers/helper-renderer-entitlements.plist" --sign "${CODESIGN_IDENT}" --deep "./RemoteFilmingStudio.app/Contents/Frameworks/OBS Helper (Renderer).app"
    ...
    codesign -dvv ./RemoteFilmingStudio.app
}
...
notarize_macos() {
    ...
    if [ -f "${FILE_NAME}" ]; then
        ...
        xcnotary precheck "./RemoteFilmingStudio.app"
    elif [ -d "RemoteFilmingStudio.app" ]; then
        NOTARIZE_TARGET="./RemoteFilmingStudio.app"
    else
        error "No notarization app bundle ('RemoteFilmingStudio.app') or disk image ('${FILE_NAME}') found"
        return
    fi
