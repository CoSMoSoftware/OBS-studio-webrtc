...
package_obs() {
    ...
    DMG_NAME=$(/usr/bin/find "${BUILD_DIR}" -type f -name "A-CAM-*.dmg" -depth 1 | sort -rn | head -1)
...
notarize_obs() {
    ...
    if [ "${NOTARIZE_IMAGE}" ]; then
        ...
        if [ -f "${OBS_IMAGE}" ]; then
            ...
            error "No notarization application bundle ('A-CAM.app') or disk image ('${NOTARIZE_IMAGE:-${FILE_NAME}}') found"
            return
        fi
    fi
...
package-obs-standalone() {
    ...
    if [ -z "${NOTARIZE_IMAGE}" -a -z "${NOTARIZE_BUNDLE}" ]; then
        if [ "${ARCH}" = "arm64" ]; then
            FILE_NAME="A-CAM-${OBS_VERSION}-macos-arm64.dmg"
        elif [ "${ARCH}" = "universal" ]; then
            FILE_NAME="A-CAM-${OBS_VERSION}-macos.dmg"
        else
            FILE_NAME="A-CAM-${OBS_VERSION}-macos-x86_64.dmg"
        fi
