...
bool OBSApp::InitGlobalConfigDefaults(const bool force /* = false */)
{
	if (force) {
		...
		config_set_bool(globalConfig, "General", "EnableAutoUpdates",
				false);
		config_set_bool(globalConfig, "General", "ConfirmOnExit", true);
#if _WIN32
...
static int run_program(fstream &logFile, int argc, char *argv[])
{
	...
	if (!isInBundle()) {
		blog(LOG_ERROR,
		     "Remote Filming cannot be run as a standalone binary on macOS. Run the Application bundle instead.");
		return ret;
	}
#endif
...
int main(int argc, char *argv[])
{
	...
	for (int i = 1; i < argc; i++) {
		if (arg_is(argv[i], "--portable", "-p")) {
			...
			std::cout << "Remote Filming Studio - "
