    ...
    ui             (new Ui::OBSBasic)
{
  ...
  stringstream name;
  name << app_name.c_str() << " " << App()->GetVersionString();
...
void OBSBasic::UploadLog(const char *file)
{
  ...
  stringstream ss;
  ss << app_name.c_str() << " " << App()->GetVersionString()
...
void OBSBasic::SystemTrayInit()
{
  trayIcon = new QSystemTrayIcon(QIcon(":/res/images/obs.png"),
      this);
  std::string whatever = app_name + " Studio";
  trayIcon->setToolTip(whatever.c_str());
    ...
    QSystemTrayIcon::MessageIcon n)
{
  if (QSystemTrayIcon::supportsMessages()) {
    QSystemTrayIcon::MessageIcon icon =
        QSystemTrayIcon::MessageIcon(n);
   std::string whatever = app_name + " Studio";
   trayIcon->showMessage(whatever.c_str(), text, icon, 10000);
  }
}
