...
static void obs_property_destroy(struct obs_property *property)
{
	...
	if (property->type == OBS_PROPERTY_BUTTON_GROUP)
...
static inline size_t get_property_size(enum obs_property_type type)
{
  ...
  case OBS_PROPERTY_BUTTON_GROUP: return sizeof(struct button_group_data);
...
static void insert_radio_button(struct button_group_data *data, size_t idx, const char *name,
		const void *val)
{
	struct list_item item = { NULL };
	item.name  = bstrdup(name);
...
void obs_property_button_group_insert_string(obs_property_t *p, size_t idx,
		const char *name, const char *val)
{
	struct button_group_data *data = get_button_group_data(p);
	if (data && data->format == OBS_BUTTON_GROUP_FORMAT_STRING)
		insert_radio_button(data, idx, name, val);
}
...
void obs_property_button_group_item_remove(obs_property_t *p, size_t idx)
{
	struct button_group_data *data = get_button_group_data(p);
	if (data && idx < data->items.num) {
		button_group_item_free(data, data->items.array+idx);
		da_erase(data->items, idx);
	}
}
...
bool obs_property_button_group_item_disabled(obs_property_t *p, size_t idx)
{
	struct button_group_data *data = get_button_group_data(p);
	return (data && idx < data->items.num) ?
		data->items.array[idx].disabled : false;
...
void obs_property_button_group_item_disable(obs_property_t *p, size_t idx,
		bool disabled)
{
	struct button_group_data *data = get_button_group_data(p);
	if (!data || idx >= data->items.num)
